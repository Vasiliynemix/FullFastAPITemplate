...
| `AUTH_JWT_ENABLED` / `GLOBAL_API_KEY_ENABLED` | см. ниже | режимы авторизации |
...
Покрыты: репозитории, сервисы, auth (JWT/сессии), декораторы, HTTP-клиент, брокер и
консьюмеры, **outbox** (релей + at-least-once), **storage** (S3 + роутер /files),
docs Basic Auth, сериализация. БД — async SQLite, Redis — fakeredis, брокер — in-memory.
