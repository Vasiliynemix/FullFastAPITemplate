...
- **Кэш ⇒ инвалидация (обязательно).** Если ручка читается из кэша (cache-aside),
  то ЛЮБАЯ мутация её данных — create/update/delete, в т.ч. из **другого сервиса** —
  обязана сбросить ключ (`cache.delete(key)`). Иначе до истечения TTL отдаётся
  устаревшее. Частая ловушка — связанные сущности: создание `Account` меняет
  `User.accounts`, поэтому `AccountService` сбрасывает `user:{id}` (ключ строит
  общая `user_cache_key`, не дублируем формат). Балансы (`deposit`/`withdraw`) и
  профиль (`upsert_profile`) — тоже. Подробнее — [ADR 0012](adr/0012-cache-invalidation.md).
