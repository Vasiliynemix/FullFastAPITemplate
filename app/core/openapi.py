...
_DESCRIPTION_TAIL = """
## Списки: пагинация, фильтры, сортировка, поиск

Параметры ниже работают на **любой ручке, отдающей список** (напр. `GET /users`).

* **Пагинация:** `?page=1&per_page=50`; в `meta` — `total`, `pages`.
* **Сортировка:** `?sort=field` (по возрастанию) или `?sort=-field` (по убыванию).
  По умолчанию — `created_at` по возрастанию.
* **Умный поиск:** `?q=строка` — терпит опечатки (находит «Вася» по запросу «всая»).
  Сортирует по релевантности. На SQLite — обычная подстрока.
* **Фильтры:** `?field__op=value`. Оператор по умолчанию `eq`
  (`?is_active=true` == `?is_active__eq=true`). Несколько фильтров — по AND.
...
| `like` | `LIKE` (регистрозависимо) | `?email__like=%@gmail.com` |
| `ilike` | `ILIKE` (регистронезависимо) | `?full_name__ilike=ив%` |
| `contains` | подстрока `ILIKE %value%` | `?full_name__contains=ив` |
| `in` | `IN (...)` через запятую | `?role__in=admin,manager` |

Неизвестное поле/оператор или неверное значение → `400`.
