...
# ---------------- Git ----------------
.PHONY: git
git: ## add + commit + push одним вызовом: make git m="сообщение"
	@test -n "$(m)" || { echo 'Использование: make git m="сообщение коммита"'; exit 1; }
	git add -A
	git commit -m "$(m)"
	git push
...
prod-init-volumes: ## Создать external-тома prod (БД, сертификаты). Идемпотентно
	@docker volume create full-fastapi_pgdata >/dev/null && echo "  ✔ full-fastapi_pgdata"
	@docker volume create full-fastapi_certbot_conf >/dev/null && echo "  ✔ full-fastapi_certbot_conf"
...
prod-destroy: ## ☠️  ОПАСНО: снести prod-стек ВМЕСТЕ С БД и сертификатами (с подтверждением кодом)
	...
	echo "    • full-fastapi_pgdata        (вся база PostgreSQL)"; \
	echo "    • full-fastapi_certbot_conf  (Let's Encrypt сертификаты)"; \
	...
	echo "  💥 Удаляю..."; \
	$(COMPOSE_PROD) down -v; \
	docker volume rm -f full-fastapi_pgdata full-fastapi_certbot_conf >/dev/null 2>&1 || true; \
...
#   make loadtest url=http://backend:8080   # МИМО nginx — настоящая ёмкость бэкенда
NET ?= full-fastapi_default
